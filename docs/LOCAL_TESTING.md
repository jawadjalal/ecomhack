...
| 15 | LLM buyer chats with the merchant (A2A) | With a key: console → `s` → **Chat (A2A)** → Send | The buyer's messages after the brief are written by the model (agent name `<model>-buyer`), stay short, respect the brief, and end in an order or a specific "No thanks: …"; if the model errors, the chat still finishes (scripted fallback, warning in the server log) |
