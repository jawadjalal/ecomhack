...
- **2026-09-26 17:05 UTC — merge #68 (animated crew mascots that follow Darwin) into main.** Kept main's layouts, copy, crew names (Darwin, Iris, Pixel, Fizz, Dash), calm idle SVGs, favicon and renderer (`components/mascots/animated-mascot.tsx`). Added #68's state logic as `lib/mascot/state.ts` (+ tests), adapted to main's crew (diagnose/"analyst" lines are Iris): page-head and empty-state faces on Issues, Fixes, Experiments, Changes, Overview cards, impact strip and Autopilot use `CrewFace`, so the crew member whose loop step runs works/thinks, a fresh ship or reject flashes success/error, and everyone sleeps with autopilot off. Ask Darwin (bottom bar and Overview chat) thinks while a reply is in flight, then flashes success or error (`components/mascots/use-chat-mascot.ts`). Classic console: activity log, loop ring and stage panel show crew mascots with live poses. Fix/issue detail, store-agent chat, tests card, personalize decisions, onboarding PR/live steps and readiness loading follow their own status. Dropped #68's duplicate renderer (`components/mascot/*`), its bouncier idle SVGs and PNG favicons (main's calm idle and `app/icon.svg` stay). Open: poses ride the 1s clock and loop polling, so a flash can be missed between polls.
