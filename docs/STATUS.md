...
- **2026-09-26 17:45 UTC — Claude (PR #42).** Merged main into the agent-team branch: OpenRouter stays the default provider, Apinex from main is kept (auto order OpenRouter → xAI → Apinex → Anthropic) and native tool calling now also runs on Apinex; xAI/Apinex → OpenRouter fallback kept. Typecheck, lint, 577 tests and build pass. Still open: agent team, per-agent chat tabs, voice mode, copy pass (see PR #42).
