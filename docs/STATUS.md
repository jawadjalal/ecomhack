...
- **2026-09-26 16:55 UTC, README agent.** Root `README.md` rewritten from scratch for today's app (loop, crew, tour of every screen, agent commerce over A2A/MCP/ACP, honesty rules, run locally, env keys, stack) with 19 new screenshots in `docs/screenshots/` (16 desktop at 1440×900, 3 phone at 390×844) taken from a local production build in demo mode (simulated shoppers, labelled). Old `apps/web/docs/screenshots/**` removed; `docs/LOCAL_TESTING.md` now points at `docs/screenshots/`. No app code touched. Still open: Research has no screenshot (the LLM summary timed out locally and fell back to a messy heuristic summary); `/volt` is undocumented.
