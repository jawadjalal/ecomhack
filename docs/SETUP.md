# Darwin: setup and technical guide

Everything technical about Darwin: URLs, keys, how each piece works, the store agent, personalization, Telegram,
agent mode, the MCP server and the CLI. For the short version, see the [README](../README.md).
...
are dry-run previews. See [docs/LOCAL_TESTING.md](LOCAL_TESTING.md) for testing with an LLM and a real GitHub token,
and [docs/DEMO.md](DEMO.md) for the 3-minute demo script.
...
| n/a | Low-stock urgency (wildcard) | humans | inconclusive, shelved |
...
2. Pick a webhook secret: 1 to 256 characters, only `A-Z`, `a-z`, `0-9`, `_` and `-` (for example `openssl rand -hex 32`).
...
### Agent mode: Cmd+K, WebMCP and `window.darwin`
...
- **Cmd+K (Ctrl+K)** anywhere in `/console`: type in plain words ("build a dashboard of coupon usage per hour for
...
[docs/posthog-extraction.md](posthog-extraction.md) for what we took from PostHog.
