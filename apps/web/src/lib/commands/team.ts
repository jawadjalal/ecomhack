/**
 * Darwin's watch commands (team area): server-only, because they call lib/team directly. Kept apart from the
 * isomorphic registry in ./specs.ts (⌘K, WebMCP, window.darwin), which can't import server code. Not yet wired
 * into specs.ts / run.ts / server-run.ts; the Inbox page itself is reachable there as the "inbox" page.
 *
