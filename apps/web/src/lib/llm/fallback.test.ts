...
describe("xAI → OpenRouter fallback", () => {
  ...
  function setup(withOpenRouter: boolean) {
    for (const k of ["APINEX_API_KEY", "ANTHROPIC_API_KEY", "OPENROUTER_MODEL", "OPENROUTER_REASONING"]) vi.stubEnv(k, "");
    // OpenRouter comes first in auto-detect, so pin Grok the way the certificate trial does.
...
describe("Apinex", () => {
  ...
  it("is picked from APINEX_API_KEY after OpenRouter and xAI", () => {
    for (const k of ["LLM_PROVIDER", "XAI_API_KEY", "APINEX_MODEL", "ANTHROPIC_API_KEY", "OPENROUTER_API_KEY"]) vi.stubEnv(k, "");
  ...
  it("runs free/gpt-6-luna when forced, and falls back to OpenRouter on a billing error", async () => {
