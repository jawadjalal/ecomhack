...
async function post<T>(path: string, body: unknown, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new TavilyError(name === "TimeoutError" || name === "AbortError" ? `Tavily ${path} timed out` : `Couldn't reach Tavily (${(e as Error).message})`);
  }
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new TavilyError(`Tavily ${path} failed: HTTP ${res.status}${text ? ` ${text.slice(0, 200)}` : ""}`);
  }
...
export async function tavilySearch(
  query: string,
  opts: { maxResults?: number; depth?: "basic" | "advanced"; includeAnswer?: boolean; timeoutMs?: number } = {},
  ...
  return { answer: typeof raw.answer === "string" ? raw.answer : undefined, results };
}

export async function tavilyExtract(urls: string[], opts: { timeoutMs?: number } = {}): Promise<TavilyExtractResult[]> {
  const list = urls.filter(isHttp).slice(0, 20);
  if (!list.length) return [];
  const raw = await post<{ results?: unknown[] }>("/extract", { urls: list }, opts.timeoutMs ?? 20_000);
  return (Array.isArray(raw.results) ? raw.results : [])
    .map((r) => r as Record<string, unknown>)
    .filter((r) => isHttp(r.url) && typeof r.raw_content === "string")
    .map((r) => ({ url: String(r.url), raw_content: String(r.raw_content).slice(0, 20_000) }));
}
