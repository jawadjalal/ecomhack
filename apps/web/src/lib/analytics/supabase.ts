...
interface MirrorState {
  ...
  /** The running flush, so concurrent callers can await the same drain. */
...
/** Flush everything queued. Concurrent calls share one drain; exported for tests and shutdown hooks. */
export function flushSupabaseMirror(): Promise<void> {
  const s = state();
  return (s.inflight ??= drain().finally(() => {
