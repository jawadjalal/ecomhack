...
export const ROADMAP: RoadmapArea[] = [
  ...
  },
  {
    ...
    done: "Impact strip (before Darwin vs now), conversion, A vs B, which agents buy, live shoppers joined to their journey, the Ask Darwin chat. The four cards sit in an even 2-up grid and share one store snapshot with the chat; numbers/bars replay their entrance animation only on real changes. Crew mascots follow what Darwin is doing (lib/mascot/state), and the Ask Darwin face thinks while a reply is in flight. Group chats open as their own tabs (Pixel, Fizz, Dash by those names).",
    leftToDo: ["Both card rows above the fold at 1440×900", "Make the chat the lead agent on every screen, able to navigate and run any action", "Persist group-chat threads on the server", "The live feed's row-reorder animation still runs on a short interval; revisit if still jittery"],
    limitations: ["People rows need store traffic on (the events route has no store-only filter).", "With ?mock=1 the cards use the in-browser engine and the chat still answers from the server.", "Group tab history is sessionStorage on this browser."],
    nextIdeas: ["Point ?mock=1 chat at the same in-browser engine as the cards", "A daily what-changed digest card", "Drive mascot poses from server events (SSE) instead of the 1s clock + loop poll"],
  },
  {
    key: "issues",
    name: "Issues",
  ...
  },
  {
    ...
    done: "Per-source and per-search page changes on any store with darwin.js, A/B tested, heatmap, autopilot. Never publishes invented claims: ideas that need a merchant fact become [Your …] drafts. The install card shows the one darwin.js tag from the server (same as onboarding and llms.txt) with Waiting / Installed / Verified status.",
    leftToDo: ["Live-tests strip above the fold", "A clear “fill in the blanks” flow for [Your …] placeholders"],
    limitations: ["Rules a merchant started by hand aren't auto-retracted.", "Onboarding and Dashboards don't yet show the same three install words (lib/web/install-status)."],
    nextIdeas: ["Use installStatus() in onboarding's verify pill and the Dashboards header"],
  },
  {
    key: "traffic",
    name: "Traffic",
    ...
    status: "done",
    done: "Where visitors come from (humans and AI agents), with insights. The site picker lists every site Darwin knows (plans ∪ rules ∪ sites with events).",
