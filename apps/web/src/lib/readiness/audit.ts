...
export async function auditWithArtifacts(input: string): Promise<{ report: ReadinessReport; artifacts: Artifacts }> {
  ...
  return { report: { ...evaluate(artifacts), checkedAt: new Date().toISOString(), durationMs: Date.now() - started }, artifacts };
}
