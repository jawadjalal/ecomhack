...
const g = globalThis as unknown as { __darwinReadinessHits?: Map<string, number[]> };
const hits = (): Map<string, number[]> => (g.__darwinReadinessHits ??= new Map());
 ...
 */
export function takeToken(bucket: string, key: string, max: number, windowMs: number, now = Date.now()): boolean {
  const map = hits();
  const k = `${bucket}:${key}`;
