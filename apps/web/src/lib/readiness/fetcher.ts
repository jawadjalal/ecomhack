...
export function blockedReason(ip: string): string | null {
  ...
  if (v6 === "::" || v6.startsWith("fc") || v6.startsWith("fd") || v6.startsWith("fe8") || v6.startsWith("fe9") || v6.startsWith("fea") || v6.startsWith("feb") || v6.startsWith("ff")) {
  ...
  try {
    for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
      ...
      const res = await fetch(url, {
        ...
        body: init.body,
        redirect: "manual",
        headers: { "user-agent": USER_AGENT, accept: "text/html,application/json,text/plain;q=0.9,*/*;q=0.5", ...init.headers },
      ...
      return { url: url.href, status: res.status, headers, body: await readCapped(res) };
    }
