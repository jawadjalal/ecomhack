...
export function BuyersLostCard({
  rows,
  selected,
  onSelect,
}: {
  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col gap-4 lg:flex-row lg:gap-5">
        <div className="flex shrink-0 flex-col lg:w-[150px]">
          <div className="flex items-baseline justify-between gap-3">
            <h2 className="text-[22px] leading-tight font-semibold tracking-[-0.02em]">
              Buyers lost
            </h2>
            <span className="text-[13px] text-[#4F4417] lg:hidden">
              per 1,000 visits
            </span>
          </div>
          <div className="mt-2 flex flex-wrap items-baseline gap-x-2.5 gap-y-1 lg:flex-col lg:items-start">
            <span className="num text-[40px] leading-none font-semibold tracking-[-0.02em]">
              {fmtImpact(total)}
            </span>
            <span className="text-[14px] leading-snug text-[#4F4417]">
              {inTest.length
          ...
          <span className="mt-auto hidden text-[13px] text-[#4F4417] lg:block">
            per 1,000 visits
          </span>
        </div>
        <div
          className="grid h-[150px] min-w-0 flex-none gap-1.5 sm:gap-2.5 lg:h-auto lg:flex-1"
          style={{
          ...
          {rows.map((r, i) => {
            ...
            return (
              <button
                ...
                aria-pressed={on}
                className="group relative flex min-h-0 min-w-0 flex-col items-center gap-1.5 rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-dw-ink"
              >
                <Tip
                  align={
                    i < 2 && rows.length > 3
                      ? "start"
                      : i >= rows.length - 2 && rows.length > 3
                        ? "end"
                        : "center"
                  }
                >
                ...
                <span className="flex min-h-0 w-full flex-1 flex-col items-center justify-end gap-1">
                  <span
                    className={cn(
                      "num text-[12px] leading-none font-semibold transition-opacity",
                      !on && selected && "opacity-70",
                    )}
                  >
                    {fmtImpact(v)}
                  </span>
                  <motion.span
                    ...
                    style={{
                      height: `calc((100% - 16px) * ${h / 100})`,
                      minHeight: 8,
                      transformOrigin: "bottom",
                    }}
                    className={cn(
                      "block w-[16px] shrink-0 rounded-full transition-[filter,box-shadow,background-color] duration-200 group-hover:brightness-75 sm:w-[22px]",
                      r.status === "test"
                        ? "bg-dw-ink"
                        : r.status === "drafted"
                ...
                <span
                  className={cn(
                    "num text-[12px] leading-none transition-colors",
                    on ? "font-semibold text-dw-ink" : "text-[#4F4417]",
                  )}
                >
                  #{r.n}
                </span>
...
export function WhoCard({
  ...
  const groups = (["Agents", "People", "Everyone"] as const)
    .map((who) => {
      ...
      return {
        who,
        list,
        lost: list.reduce((n, r) => n + r.insight.impactScore, 0),
      };
    })
  ...
  const tie =
    sorted.length > 1 &&
    Math.abs(sorted[0].lost - sorted[1].lost) / total < 0.06;
            ...
            >
              ...
              <span className="relative block h-3 w-full rounded-full bg-dw-ink/12">
                {g.lost > 0 && (
                  <motion.span
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.max(4, w * 100)}%` }}
                    transition={{
                      duration: 0.8,
                      delay: 0.2 + i * 0.1,
                      ease: EASE,
...
export function WhereCard({
  ...
  const starts = cols.map((_, i) =>
    cols.slice(0, i).reduce((n, c) => n + Math.min(MAX_DOTS, c.list.length), 0),
  );
    ...
    >
      ...
      {top && top.lost > 0 && (
        <p className="mt-1.5 text-[14px] leading-snug text-[#2F3517]">
          The {top.label.toLowerCase()}{" "}
          {top.key === "home"
            ? "page"
            : top.key === "product"
              ? "pages"
              : "step"}{" "}
          lose{top.key === "product" ? "" : "s"} the most buyers.
        </p>
      )}
      <div
        className="mt-auto grid grid-cols-4 items-end gap-2 pt-3"
        onMouseLeave={() => onFocus(null)}
      >
        {cols.map((c, ci) => {
          ...
          return (
            ...
            >
              <Tip>
                {c.list.length
                  ? `${plural(c.list.length, "issue")} · ${fmtImpact(c.lost)} lost`
                  : "No issues"}
              </Tip>
              {more > 0 && (
                <span className="num text-[11px] leading-none font-semibold text-[#2F3517]">
                  +{more}
                </span>
              )}
              <div className="flex flex-col-reverse items-center gap-1">
                {shown.map((r, di) => {
                  ...
                  return (
                    <motion.button
                      ...
                      aria-pressed={on}
                      className={cn(
                        "size-4 rounded-full outline-none focus-visible:ring-2 focus-visible:ring-white",
                        r.status === "test"
                          ? "bg-dw-ink"
                          : r.status === "drafted"
                            ? "border-[1.5px] border-dashed border-dw-ink bg-dw-ink/20"
                            : "border-[1.5px] border-dashed border-dw-ink",
                ...
                {!c.list.length && (
                  <span
                    className="size-4 rounded-full border border-dw-ink/15"
                    aria-hidden
                  />
                )}
              </div>
              <span className="text-[12px] leading-none text-[#2F3517]">
                {c.label}
              </span>
            </div>
          );
