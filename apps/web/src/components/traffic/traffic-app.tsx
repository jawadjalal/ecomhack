...
import { Card, CardTitle, Empty, LegendKey, PageHead, PillButton, Tag, type Tone } from "@/components/dw/ui";
import { DwSwitch, HEAD_CONTROLS, SiteSelect } from "@/components/dw/personalize/kit";
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)]">
        ...
        <Card tone="blue" shape="observer" corner="br">
          <CardTitle>Visit the store as if you came from…</CardTitle>
...
function DimensionPanel({
  ...
  return (
    <Card tone={tone} shape={shape} corner="br" hover={false}>
      <CardTitle right={rows?.length ? <span>visitors · ordered</span> : undefined}>{title}</CardTitle>
...
function InsightsPanel({ site, synthetic, visitors }: { site: string; synthetic: boolean; visitors: number }) {
  ...
  return (
    <Card tone="white" hover={false}>
      <CardTitle
        right={
          <PillButton size="sm" onClick={() => ask(true)} disabled={!!busy} title="Send this report to Darwin's AI model (Grok / Claude) for suggestions">
