...
function InsightsPanel({ site, synthetic, visitors }: { site: string; synthetic: boolean; visitors: number }) {
  ...
  return (
    <Card tone="white" hover={false}>
      <CardHead
        right={
          <PillButton size="sm" onClick={() => ask(true)} disabled={!!busy} title="Send this report to Darwin's AI for suggestions">
      ...
      {res && (
        <p className="mt-2 text-[13.5px] text-dw-ink/60">
          {res.source === "llm" ? "Written by Darwin's AI" : "Built-in rules"}
